        ...
        if(listSamples[i].trainOrTest==1 && listSamples[i].filterSample==0){
            ...
            for(int j = 0; j < bestNumberOfAreas; j++){
                ...
                for(int k = 0; k < bestAreas[j].numCases; k++)
                {
                    ...
                    if(epsilonIsMinRadius)
                        tGotcha = std::min(oneLastFinal[bestAreas[j].caseIDs[k]], radius) > 
                            distanceMatrix[i][bestAreas[j].caseIDs[k]];
                    else tGotcha = oneLastFinal[bestAreas[j].caseIDs[k]] > distanceMatrix[i][bestAreas[j].caseIDs[k]];

                    if(tGotcha)
                        ctr+=((double)coreN[bestAreas[j].caseIDs[k]])/minpts;
                    gotcha = ctr > lim;
                    if(gotcha){
